...
# global dictionnary
recordroute = "Record-Route: <sip:%s:%d;lr>" % (socket.gethostbyname(socket.gethostname()), 5060)
topvia = "Via: SIP/2.0/UDP %s:%d" % (socket.gethostbyname(socket.gethostname()), 5060)
registrar = {}
    ...
    def processRegister(self):
        ...
        if rx_invalid.search(contact) or rx_invalid2.search(contact):
            if fromm in registrar:
                del registrar[fromm]
            self.sendResponse("488 Neakceptovatelne")
            return
...
def initSIP():
    ...
    print("Server spusteny na: " + str(ipaddress))
